      ...
      <ul className="divide-y divide-gray-100 rounded-md border border-gray-200">
        {attachments?.map((att) => (
          <li key={att.id} className="flex items-center justify-between px-3 py-2 text-sm">
            <button onClick={() => handleDownload(att.id, att.filename)} className="truncate text-left text-blue-600 hover:underline">
              {att.filename}
            </button>
            <div className="flex shrink-0 items-center gap-2 text-xs">
              {card.cover_attachment_id === att.id ? (
                <button onClick={() => setCover.mutate(null)} className="text-gray-500 hover:text-gray-800">
                  Remove cover
                </button>
              ) : (
                <button onClick={() => setCover.mutate(att.id)} className="text-gray-500 hover:text-gray-800">
                  Set as cover
                </button>
              )}
              <button onClick={() => remove.mutate(att.id)} className="text-gray-400 hover:text-red-600">
                Delete
              </button>
            </div>
          </li>
        ))}
        {attachments?.length === 0 && <li className="px-3 py-2 text-sm text-gray-400">No attachments yet.</li>}
      </ul>
    </div>
  );
}
