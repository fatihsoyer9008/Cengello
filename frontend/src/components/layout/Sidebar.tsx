...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-full w-60 shrink-0 flex-col overflow-y-auto border-r border-gray-200 bg-white px-2 py-3 transition-transform duration-200 dark:border-white/10 dark:bg-[#1d2125] md:static md:z-auto md:translate-x-0 ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <nav className="space-y-0.5" onClick={onClose}>
        ...
        <div className="mt-1 space-y-0.5" onClick={onClose}>
          {workspaces?.map((ws) => (
            <Link
              key={ws.id}
